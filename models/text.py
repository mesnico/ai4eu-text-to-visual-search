...
def EncoderText(config):
    ...
    if config['text-model']['name'] == 'gru' or config['text-model']['name'] == 'lstm':
        ...
        bidirectional = config['text-model']['bidirectional']
        model = EncoderTextGRU(vocab_size, word_dim, embed_size, num_layers, order_embeddings=order_embeddings,
                               bidirectional=bidirectional, type=config['text-model']['name'])
...
class EncoderTextGRU(nn.Module):
    ...
    def __init__(self, vocab_size, word_dim, embed_size, num_layers,
                 order_embeddings=False, bidirectional=False,
                 type='gru'):
        ...
        # caption embedding
        if type == 'gru':
            print('Using GRU as language model')
            self.rnn = nn.GRU(word_dim, embed_size // 2 if bidirectional else embed_size, num_layers, batch_first=True,
                              bidirectional=bidirectional)
        elif type == 'lstm':
            print('Using LSTM as language model')
            self.rnn = nn.LSTM(word_dim, embed_size // 2 if bidirectional else embed_size, num_layers, batch_first=True,
                               bidirectional=bidirectional)
        self.init_weights()
